...
class SongCache {
   ...
   * @param {Object} noteSequence - The NoteSequence to save
   ...
   * @returns {Object|null} - The most recent NoteSequence or null if none found
   */
  getLatestSong() {
    try {
   ...
   * @returns {Array} - Array of song information objects
   */
  getSongList() {
    try {
      const files = fs.readdirSync(this.cacheDir)
   ...
   * @returns {Object|null} - The NoteSequence or null if not found
   */
  getSong(filename) {
    try {
      const filepath = path.join(this.cacheDir, filename);
